...
class INRFittingSystem(L.LightningModule):
    ...
    def on_validation_epoch_end(self):
        ...
        scores_fitting = metrics(all_gt, all_pred, prefix="val_fitting")
        ...
        if self.decoder:
            ...
            scores_recon = metrics(all_raw, all_recons, prefix="val_recon")
...
def train_inr(configs):
    ...
    pipeline_configs.inr.dim_in = 3 if dataset_configs.type == "ST3D" else 2
    ...
    fitting_system = INRFittingSystem(pipeline_configs, val_pca=val_pca, decoder=decoder)
    ...
    if pipeline_configs.target == "embeddings": # additionally log embeddings
        train_embd_fig, val_embd_fig = dataset.plot_embeddings(train_indices=train_idx, val_indices=val_idx)
        tb_logger.experiment.add_figure("dataset/train_embd", train_embd_fig)
        tb_logger.experiment.add_figure("dataset/val_embd", val_embd_fig)
